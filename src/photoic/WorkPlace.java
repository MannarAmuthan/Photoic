...
import StatePackage.StateBrush;
import StatePackage.StateEraser;
import StatePackage.StateMove;
import StatePackage.StateColorPicker;
import StatePackage.StatePen;
import StatePackage.StateShapeElipse;
import StatePackage.StateShapeRect;
import StatePackage.StateText;
...
public class WorkPlace {
    ...
    if(newState==MouseState.MOVE){state=new StateMove(this);mouseState=MouseState.MOVE;}
    if(newState==MouseState.PENTOOL){state=new StatePen(this);mouseState=MouseState.PENTOOL;}
    if(newState==MouseState.BRUSH){state=new StateBrush(this);mouseState=MouseState.BRUSH;}
    if(newState==MouseState.ERASER){state=new StateEraser(this);mouseState=MouseState.ERASER;}
    if(newState==MouseState.S_RECT){state=new StateShapeRect(this);mouseState=MouseState.S_RECT;}
    if(newState==MouseState.S_ELI){state=new StateShapeElipse(this);mouseState=MouseState.S_ELI;}
    if(newState==MouseState.TEXT){state=new StateText(this);mouseState=MouseState.TEXT;}
    if(newState==MouseState.PAINTFILL){state=new StateColorPicker(this);mouseState=MouseState.PAINTFILL;} 
